(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/resources/Script/view/games/Tiaoyitiao/Tiaoyitiao.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '9bd4cdLl4xGK4ElNawA9hno', 'Tiaoyitiao', __filename);
// resources/Script/view/games/Tiaoyitiao/Tiaoyitiao.js
...
cc.Class({
    ...
    onBackClicked: function onBackClicked(customEventData) {
    ...
    onLoad: function onLoad() {
        ...
        // 网络监听列表
        var listenerList = [[events.game.GAME_DATA, this.onReceive_login, this], [events.game.tiaoyitiao.S2C_START, this.onReceive_start, this], [events.game.tiaoyitiao.S2C_PLAY, this.onReceive_play, this], [events.game.tiaoyitiao.S2C_PROGRESS, this.onReceive_progress, this], [events.game.tiaoyitiao.S2C_GAMEOVER, this.onReceive_overGame, this]];
    ...
    //创建块
    createNode: function createNode(nNextId, callback) {
        ...
        // 创建阴影
        var funcCreateShadow = function funcCreateShadow(params) {
    ...
    //创建小人
    createPersonNode: function createPersonNode() {
        // 对方小人
    ...
    // 计算坐标
    calcPostion: function calcPostion(nNextId) {
        if (nNextId == 0) {
    ...
    // 移动到中点
    moveNodeToCenter: function moveNodeToCenter(nNextId) {
    ...
    // 按压调整
    touchPress: function touchPress(isPress) {
    ...
    // 播放动画音效
    playAnimationSound: function playAnimationSound(animationName, isSelf) {
    ...
    //创建帧动画
    createAnimationByName: function createAnimationByName(animName, speed, repeatCount, callback, isSelf) {
    ...
    // 箱子压缩
    updateBoxScaleY: function updateBoxScaleY(count) {
    ...
    // 发射弹力箱子
    runBoxEffect: function runBoxEffect() {
        var node = this.tableNode.getChildByTag(this.myProgressId);
    ...
    // 计算跳跃数据
    getJumpData: function getJumpData(distance, gameDataIndex, isSelf) {
    ...
    // 计算分数
    calcScore: function calcScore(nJumpSuccess, isAtCenter) {
    ...
    // 跳跃
    jumpEffect: function jumpEffect(startPos, endPos, isSelf, direction, jumpResult) {
    ...
    // 计算跳跃,是否成功
    checkJumpSuccess: function checkJumpSuccess(endPos, isSelf, jumpResult) {
        ...
        if (jumpResult.success == 0) {
            if (isSelf) {
                ...
                // 显示白点
                var self = this;
                var funcShowMidPoint = function funcShowMidPoint() {
                    // 中心点
    ...
    // 等待2s 特殊图案加分
    addExtraScore: function addExtraScore() {
        // 检测是否是 特殊图案，停留可加分
    ...
    // 设置分数
    updateTotalScore: function updateTotalScore(nMyScore, nOtherScore) {
        if (nMyScore) {
    ...
    // 加分数
    runAddScoreAction: function runAddScoreAction(nTotalScore, nCurScore) {
    ...
    // 失败动画
    runLostAction: function runLostAction(isSelf, endPos) {
    ...
    // 创建白点
    setMidPoint: function setMidPoint(index) {
    ...
    // 画线
    drawBezier: function drawBezier(startPos, endPos, controlPos) {
        if (!this.drawNode) {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=Tiaoyitiao.js.map
